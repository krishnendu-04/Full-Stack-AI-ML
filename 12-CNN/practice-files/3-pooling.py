...
# Max Pooling
max_pool_tensor = tf.keras.layers.MaxPooling2D(pool_size=(2,2), strides=2, padding='valid')
max_pooled_tensor = max_pool_tensor(input_tensor)

# Average Pooling
avg_pool_tensor = tf.keras.layers.AveragePooling2D(pool_size=(2,2),strides=2, padding='valid')
avg_pooled_tensor = avg_pool_tensor(input_tensor)
...
input_pytorch = torch.tensor(feature_map, dtype=torch.float32).unsqueeze(0).unsqueeze(0)

# Max Pooling
max_pool_pytorch = nn.MaxPool2d(kernel_size=2,stride=2)
max_pooled_pytorch = max_pool_pytorch(input_pytorch)

# Average Pooling
avg_pool_pytorch = nn.AvgPool2d(kernel_size=2,stride=2)
avg_pooled_pytorch = avg_pool_pytorch(input_pytorch)

print(f"Max Pooled PyTorch: \n {max_pooled_pytorch.squeeze().numpy()}")
print(f"Average Pooled PyTorch: \n {avg_pooled_pytorch.squeeze().numpy()}")



# Tensorflow combine example
